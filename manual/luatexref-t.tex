...
\NC \type{\mathchardef} \NC 21=15   \NC \hex10FFFF=15   \NC ($2^{20}+2^{16}$) = ($2^{3}*2^{8}*2^{4}$) \NC\NR
...
\callback{input_line} callback. This will be explained in a later chapter.
...
\NC \type{\omathchardef}\NC 21=27   \NC \hex10FFFF=27   \NC ($2^{20}+2^{16}$) = ($2^{3}*2^{16}*2^{8}$)\NC\NR
...
States are never closed, except when a fatal (out of memory) error
occurs, at which point \LUATEX\ will exit anyway.
...
\subsection{\primitive{luastartup}} 

This integer register can be set inside the format file to specify the
lua instance number (state) in which a possible run-time \type{startup.lua}
script is executed (initex always uses the state 0)
\startsyntax
  \luastartup <16-bit number>
\stopsyntax
...
If such a file exists, it is loaded in the lua state zero
(for initex) or the one pointed to by \primitive{luastartup}
(for non-initex).
...
The texio library has its own (partial) set of I/O commands, because
the \LUATEX\ internals cannot deal directly with the standard file
handles that are returned by io.open. 

Except for this slightly different file type, the syntaxes are 
identical to their cousins from the standard I/O library. The list of
implemented functions is:
...
\starttyping
   boolean success, string actual_name, tex_file filep = 
...
the true filename used. Make sure you include the
extension. 

\sym{filep}  

the file object returned by texio.open()
\stopitemize

\subsubsection{\callback{open\_write\_file}}

Your callback function should have the same conventions as for
\callback{open_read_file}.

\subsubsection{\callback{input\_line}}

From the next input line onwards, \LUATEX\ will run this function
whenever it needs a new input line from a physical file.

\startfunctioncall
string line = function (texio_file f) 
\stopfunctioncall

Your function will receive a file handle as argument, and should
return either a string or `nil'. The value `nil' signals that the end
of file has occurred, and will make \TEX\ close the file handle.  Do
not close the file handle yourself!
