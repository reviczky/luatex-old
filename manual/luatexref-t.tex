...
\NC encodingname       \NC no       \NC no       \NC no  \NC string \NC encoding (file) name\NC\NR
...
\NC fullname           \NC no       \NC no       \NC no  \NC string \NC actual (PostScript) name\NC\NR
...
\item The virtual font commands in 'define_font' lua tables that accept 
numeric arguments are probably not correct yet. There is a conversion
factor (linked to the font designsize) that seemingly has to be taken 
into account.

\item Font expansion is currently non-functional due to massive 
changes in the virtual font handling.

\item Left-boundary ligature processing is broken, because of a change 
in the internal font structure.
