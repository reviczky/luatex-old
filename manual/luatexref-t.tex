...
private use block that starts at index 1.113.856 (10FF00). When
...
\LUATEX: \lua{slnunicode}, \lua{luazip}, \lua{luafilesystem},
\lua{lpeg} (version 0.6), and \lua{md5}.
...
The \lua{string} library has six extra iterators that return strings
piecemeal:
...
type of node. If you want to the the valid fields for a \quote{whatsit},
...
\NC pool_size \NC 	        number\NC  100000\NC cf.\ web2c docs \NC \NR
...
\NC buf_size \NC 	        number\NC  3000\NC cf.\ web2c docs \NC \NR
...
\luatex{read_otf} for loading of information from \POSTSCRIPT|-|based \OPENTYPE\
and \luatex{read_ttf} for loading of \TRUETYPE|-|based \OPENTYPE\ (or simply a
...
\NC table_version                \NC number   \NC indicates the \luatex{read_otf()} version\NC\NR
...
\starttabulate[|lT|l|p|]
\NC \bf key            \NC \bf type \NC \bf explanation \NC\NR
\NC type                   \NC enum     \NC allowed values: \type {null}, \type {position}, \type {pair},
                                        \type {substitution}, \type {alternate}, \type {multiple}, \type {ligature},
                                        \type {lcaret},  \type {kerning}, \type {vkerning}, \type {anchors},
                                        \type {contextpos}, \type {contextsub}, \type {chainpos}, \type {chainsub},
                                        \type {reversesub}, \type {max}, \type {kernback}, \type {vkernback}\NC\NR
\NC tag                    \NC string   \NC \NC\NR
\stoptabulate
...
\subsubsection{pdf_snap_ref_point nodes}
...
\subsubsection{pdf_snapy_comp nodes}
...
\item There is interference between rules and \tex{pdfliteral}. This
is also likely related to the bidirectional algorithm.

\item The \TFM\ loader sometimes stores a full path instead of only a
name, this prevents \VF\ loading from working properly.

\item The Lua setting of \type{false_boundarychar=65536} gives an error.
...
\item Move the \quote{raw bytes} outside of the legal \UNICODE\ range, to
prevent collisions.
