...
be used. All these encoding-related functions are superseded by a
\LUA-based solution (\type{reader} callbacks).
...
\NC cidregistry                  \NC string   \NC \NC\NR
...
\NC cidversion                   \NC number   \NC \NC\NR
...
already be defined in \TeX, but have not been accessed at all since
...
You can test for the status of a font calling this function:
...
\NC boundarychar       \NC no       \NC yes      \NC yes \NC number \NC right boundary character (default: 0)\NC \NR
\NC boundarychar_label \NC no       \NC yes      \NC yes \NC number \NC (this is a relic, will be removed)\NC \NR
...
\NC false_boundarychar \NC no       \NC yes      \NC yes \NC number \NC (this is a relic, will be removed)\NC \NR
...
\NC index             \NC no       \NC no       \NC yes \NC number \NC the (opentype) font glyph index\NC\NR
...
The \type{type} field inside a ligature is the numerical value of one
of the eight possible ligature types supported by \TeX.  When \TEX\
inserts a new ligature, it puts the new glyph in the middle of the
left and right glyphs. The original left and right glyphs can
optionally be retained, and when at least one of them is kept, it
is also possible to move the new `insertion point' forward one or two
places. The glyph that ends up to the right of the insertion point
will become the next `left'.

\def\|{\string|}
\def\>{\string>}
\starttabulate[|l|l|p|]
\NC textual (Knuth)      \NC value         \NC result (\| = final `insertion point')\NC\FR
...
font, and the inclusion mechanism will attempt to add the proper
definitions to the PDF.
...
version of \LUATEX: If a `real' font is using character metrics above
the value 255, then it is assumed to be a wide font. A more lenient
approach is often possible, and will perhaps be implemented later.
...
\type{encodingname}, \type{filename}, and the \type{index} key in 
...
Only \type{truetype} and \type{opentype} fonts can be `wide' fonts.
On top of that, \type{truetype} format requires the
\type{\pdfminorversion} to be at least~6, otherwise an invalid PDF would 
be produced.
...
PostScript/PDF font name. The \type{encodingname} will be used as the
character set (the CID \type{/Ordering} key). The
...
The \type{fonts} is another Lua array. The values are two-value arrays
themselves, each entry indicating one of the base fonts in a virtual
font. An example makes this  easy to understand
...
\type{ptrmr8a.tfm} loaded at 10pt, and the second is \type{psyr.tfm} 
loaded at a little over 9pt. These index numbers are used by the
character command definitions that are part of each character.
...
PDF mode yet. This is especially true for the Top-Bottom and
Bottom-Top writing directions, but it there are also some remaining
problems with Right-Left.

\item Font expansion is currently non-functional due to massive 
changes in the virtual font handling.

\item Left- and rightboundary ligature processing is broken, because of
a change in the internal font structure.
