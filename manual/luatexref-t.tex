...
If the Lua script can not be found using the actual name of the
command||line, a second attempt is made by prepending the value of the
environment variable \type{LUATEXDIR}, if it esists.

Then the script is loaded and executed. It will find the entire
commandline in the table \type{arg}, beginning with \type{arg[0]},
that is the name of the executable. At the end of script execution,
\LUATEX\ will fetch some of the processed command line options 
from the \type{texconfig} table  (see the description of the
\type{texconfig} table later on in this document).
...
you can at least report your actions to the log file when it
...
Two modules that are normally external are statically linked in with
\LUATEX: \type{slnunicode} and \type{luazip}.
...
\item The number of fonts, ocps and ocplists is capped off at about 8\% 
of their maximum \ALEPH\ value (around 5000 for each).
