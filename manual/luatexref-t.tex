...
\subsection{\primitive{noligkerns}}

This primitive prohibits ligature and kerning insertion at the time
...
\subsubsubject{\function{pdf.print}}
...
into the matching \callback{read_file} callback.
...
value to nil). See the description of \callback{token} for some
...
\subsubsection{\callback{buildpage_filter}}
...
\startfunctioncall
     function (table <nodelist>)
        return table <nodelist>
     end
\stopfunctioncall

A \type{nodelist} is a list of hashes, each representing a single node
in the list. The node format is another list, and it varies depending
on the node type. Regardless of that, the first three items always
have the same semantics:

\starttabulate[|l|l|l|]
\NC ~ \NC type    \NC  Explanation \NC\FR
\NC 1 \NC string  \NC  The node's \type{type}, as a string\NC\NR
\NC 2 \NC number  \NC  The node \type{subtype} identifier\NC\NR
\NC 3 \NC boolean \NC  A true/false value you can use as you feel fit\NC\NR
\stoptabulate

The first entry is a controlled list of keywords. The complete list
is: "hlist", "vlist", "rule", "ins", "mark", "adjust", "ligature",
"disc", "whatsit", "math", "glue", "kern", "penalty", "unset",
"glyph", "margin_kern" (these can happen in horizontal and vertical
lists), as well as: "style", "choice", "ord", "op", "bin", "rel",
"open", "close", "punct", "inner", "radical", "fraction", "under",
"over", "accent", "vcenter", "left", "right" (those only happen in
math lists).

At the moment, the node representation is rather cryptic. A helper
routine to convert this compact representation of a node to and from a
dictionary-style table is planned, but not ready yet.

\subsubsection{\callback{pre_linebreak_filter}}

This callback is called just before \LUATEX\ starts converting a list
of nodes into a stack of \type{\hbox}es. The skip removal and
insertion of \type{\parfillskip} has not happend yet.

\startfunctioncall
     function (table <nodelist>, string <groupcode>)
        return table <nodelist>
     end
\stopfunctioncall
...
\callback{pre_linebreak_filter}, some are for the
\callback{Xpack_filter} callbacks that will be explained in the next
two paragraphs.
...
\NC insert\NC       \NC\NR
\NC vcenter\NC       \NC\NR
\NC local_box\NC       \NC\NR
\NC split_off\NC       \NC\NR
\NC split_keep\NC       \NC\NR
\NC preamble\NC       \NC\NR
\NC align_set\NC       \NC\NR
\NC fin_row\NC       \NC\NR

\stoptabulate

\subsubsection{\callback{hpack_filter}}
...
\startfunctioncall
     function (table <nodelist>, number <size>, string <packtype>,
     string <groupcode>) return table <nodelist> end
\stopfunctioncall

The \type{packtype} is either `additional' or `exactly'. If
`additional', then the \type{size} is a \type{\hbox spread ...}
argument. If `exacty', then the \type{size} is a \type{\hbox to ...}.
In both cases, the number is in scaled points.

\subsubsection{\callback{vpack_filter}}

This callback is called when \TEX\ is ready to start boxing some
vertical mode material. Math items are ignored at the moment.

This function is very similar to the \callback{hpack_filter}. Besides
the fact that it is called at different moments, there is an extra
variable that matches \TEX's \type{\maxdepth} setting.
...
+ Fix extensible character handling 
+ Fix for contextual lookup tags in otf
+ TTF language names added
+ Nicer VF error
+ Font subset prefix calculation fix
+ Various patches from David
+ Solaris 9 patches from Arthur