...
\luacallback{reader} callback. This will be explained in a later chapter.
...
and can be used after \type{\the} etc. just like the normal
\type{\count} registers.
...
to the primitives \texprimitive{directlua} and \texprimitive{latelua}.

\subsection{\texprimitive{directlua}}

The primitive \texprimitive{directlua} is used to execute lua code immediately.
The syntax is
\startsyntax
...
\subsection{\texprimitive{latelua}}

\texprimitive{latelua} stores lua code in a whatsit that will be processed 
...
\subsection{\texprimitive{luaescapestring}}
...
\subsection{\texprimitive{closelua}} 
...
\subsection{\texprimitive{clearmarks}}
...
\subsection{\texprimitive{noligs} and \texprimitive{nokerns}}
...
\subsection{\texprimitive{formatname}}

\texprimitive{formatname}'s syntax is identical to \type{\jobname}. 
...
\subsection{\texprimitive{scantextokens}}

The syntax of \texprimitive{scantextokens} is identical to \type{\scantokens}. 
...
\subsubsection{\texprimitive{catcodetable}}
\startsyntax
  \catcodetable <28-bit number>
\stopsyntax

The \texprimitive{catcodetable} switches to a different catcode table.
...
\subsubsection{\texprimitive{initcatcodetable}}
\startsyntax
  \initcatcodetable <28-bit number>
\stopsyntax

The \texprimitive{initcatcodetable} creates a new table with catcodes
identical to those defined by initex:

\starttable[|l|l|l|]
\NC \type{^^M}~(<return>) \NC \type{car_ret}     \NC5 \NC\NR
\NC \type{ }~(space)      \NC \type{spacer}      \NC10\NC\NR
\NC {\tt\textbackslash }  \NC \type{escape}      \NC0 \NC\NR
\NC \type{%}              \NC \type{comment}     \NC14\NC\NR
\NC \type{^^?}~(<delete>) \NC \type{invalid\_char}\NC15\NC\NR
...
\subsubsection{\texprimitive{savecatcodetable}}
\startsyntax
  \savecatcodetable <28-bit number>
\stopsyntax

\texprimitive{savecatcodetable} copies the current set of catcodes to a
...
the \type{tex},\type{token}, \type{node} and \type{pdf} tables are off-limits
...
\type{tex}, \type{token}, \type{node} and \type{pdf} tables: those 
...
\type{lpeg} (version 0.6), and \type{md5}.
...
piecemeal: 
\startitemize
\item \type{string.utfvalues(s)} (returns an integer value in the
unicode range)
\item \type{string.utfcharacters(s)} (returns a string with a single
UTF-8 token in it)
\item \type{string.characters(s)} (a string of length one)
\item \type{string.characterpairs(s)} (two strings of length one) will
produce an empty second string in the string length was odd.
\item \type{string.bytes(s)} (a single byte value) 
\item \item{string.bytepairs(s)} (two byte values) Will produce nil instead of a
...
\subsection{attribute, count, dimension and token registers}
 ...
 \TeX's attributes (\type{\attribute}), counters (\type{\count}),
 dimensions (\type{\dimen}) and token (\type{\toks}) registers can be
...
to use a valid \type{\countdef} (or \type{\attributedef}, or
\type{\dimendef}, or \type{\toksdef}), anything else will generate an error 
...
interface as defined in the \lualib{node} library:
...
\type{\box2} will be cleared by \TeX\ commands later on, the contents
of \type{\box0} becomes invalid as well. To prevent this from
happening, always use \type{node.copy_list()} unless you are 
...
\subsubsection{\luafunction{tex.print}}
...
\subsubsection{\luafunction{tex.sprint}}
...
\subsubsection{\luafunction{tex.write}}
...
\subsection{\luafunction{token.get_next}}
...
\subsection{\luafunction{token.is_expandable}}
...
\subsection{\luafunction{token.expand}}
...
\subsection{\luafunction{token.is_activechar}}
...
\subsection{\luafunction{token.create}}
...
\subsection{\luafunction{token.command_name}}
...
\subsection{\luafunction{token.command_id}}
...
\subsection{\luafunction{token.csname_name}}
...
\subsection{\luafunction{token.csname_id}}
...
\section{The \lualib{node} library}

The \lualib{node} library contains functions that facilitate dealing
with (lists of) nodes and their values. 

\LUATEX\ nodes are represented in Lua as userdata with
the metadata type \type{luatex.node}. The various fields within 
...
\item The \type{id} indicates \TEX's `node type'. The field \type{id} 
...
with `whatsits', because they are differentiated solely based on their 
\type{subtype}.
\stopitemize

The other available fields depend on the \type{id} (and for `whatsits', the
\type{subtype}) of the node. Further details on the various fields and their
meanings are given in the \in{chapter}[nodes].

\TEX's math nodes are not yet supported, Sso, there is not yet an interface 
to the internals of the math lists, and it is not possible to create
...
\subsubsection{\luafunction{node.types}}
...
\subsubsection{\luafunction{node.whatsits}}

\startfunctioncall
  table t = node.whatsits()
\stopfunctioncall
 
\TEX's `whatsits' all have the same \type{id}. The various subtypes
are defined by their \type{subtype}. The function is much like 
\type{node.id}, except that it provides an array of \type{subtype} 
mappings.

\subsubsection{\luafunction{node.id}}
...
\subsubsection{\luafunction{node.subtype}}
...
\subsubsection{\luafunction{node.type}}
...
\subsubsection{\luafunction{node.fields}}
...
type of node. If you want to the the valid fields for a `whatsit',
...
\subsubsection{\luafunction{node.new}}
...
\subsubsection{\luafunction{node.free}}
...
\subsubsection{\luafunction{node.flush_list}}
...
\subsubsection{\luafunction{node.copy}}
...
\subsubsection{\luafunction{node.copy_list}}
...
\subsubsection{\luafunction{node.slide}}
...
\subsubsection{\luafunction{node.length}}
...
\subsubsection{\luafunction{node.count}}
...
\subsubsection{\luafunction{node.traverse}}
...
\subsubsection{\luafunction{node.traverse_id}}
...
starts at \type{n} that have an \type{id} matching \type{tpe}.  If
...
\subsubsection{\luafunction{node.has_attribute}}
...
\subsubsection{\luafunction{node.set_attribute}}
...
\subsubsection{\luafunction{node.unset_attribute}}
...
\subsubsection{\luafunction{texio.write}}
...
\subsubsection{\luafunction{tex.write_nl}}
...
Like \luafunction{texio.write}, but make sure that the string s will
...
that can be used from within a \type{\latelua} argument. This function
is not to be used inside \type{\directlua} unless you know {\it
exactly} what you are doing.

\subsubsubject{\luafunction{pdf.print}}
...
\subsubsection{\luacallback{find_read_file} and \luacallback{find_write_file}}
...
\subsubsection{\luacallback{find_font_file}}
...
\subsubsection{\luacallback{find_output_file}}
...
\subsubsection{\luacallback{find_format_file}}
...
\subsubsection{\luacallback{find_vf_file}}

Like \luacallback{find_font_file}, but for virtual fonts. This applies to
both Aleph's \type{ovf} files and traditional Knuthian \type{vf} files.

\subsubsection{\luacallback{find_ocp_file}}

Like \luacallback{find_font_file}, but for ocp files.

\subsubsection{\luacallback{find_map_file}}

Like \luacallback{find_font_file}, but for map files.

\subsubsection{\luacallback{find_enc_file}}

Like \luacallback{find_font_file}, but for enc files.


\subsubsection{\luacallback{find_sfd_file}}

Like \luacallback{find_font_file}, but for subfont definition files.


\subsubsection{\luacallback{find_pk_file}}

Like \luacallback{find_font_file}, but for pk bitmap files. The argument
...
\subsubsection{\luacallback{find_data_file}}

Like \luacallback{find_font_file}, but for embedded files (\type{\pdfobj file "..."}).

\subsubsection{\luacallback{find_opentype_file}}

Like \luacallback{find_font_file}, but for opentype font files.

\subsubsection{\luacallback{find_truetype_file} and \luacallback{find_type1_file}}
...
into the matching \luacallback{read_file} callback.

Strangely enough, \type{find_type1_file} is also used for OpenType
(otf) fonts.

\subsubsection{\luacallback{find_image_file}}
...
\subsubsection{\luacallback{open_read_file}}
...
the filename returned by a previous \luacallback{find_read_file} or the return
...
`\luacallback{reader}' and the associated function will be called once
for each new line to be read, the optional one is `\luacallback{close}'
...
\subsubsubsection{\luacallback{reader}}
...
the optional `\luacallback{close}' function next.

\subsubsubsection{\luacallback{close}}
...
\subsubsection{\luacallback{read_font_file}}
...
\subsubsection{\luacallback{read_vf_file}}

Like \luacallback{read_font_file}, but for virtual fonts.

\subsubsection{\luacallback{read_ocp_file}}

Like \luacallback{read_font_file}, but for ocp files.

\subsubsection{\luacallback{read_map_file}}

Like \luacallback{read_font_file}, but for map files.

\subsubsection{\luacallback{read_enc_file}}

Like \luacallback{read_font_file}, but for enc files.


\subsubsection{\luacallback{read_sfd_file}}

Like \luacallback{read_font_file}, but for subfont definition files.


\subsubsection{\luacallback{read_pk_file}}

Like \luacallback{read_font_file}, but for pk bitmap files.

\subsubsection{\luacallback{read_data_file}}

Like \luacallback{read_font_file}, but for embedded files (\type{\pdfobj file "..."}).


\subsubsection{\luacallback{read_truetype_file}}

Like \luacallback{read_font_file}, but for truetype font files. The
\type{name} is a path name as returned by \luacallback{find_truetype_file} 
or \type{kpse_find_file}.

\subsubsection{\luacallback{read_type1_file}}

Like \luacallback{read_font_file}, but for type1 font files. The
\type{name} is a path name as returned by \luacallback{find_type1_file} 
or \type{kpse_find_file}.

\subsubsection{\luacallback{read_opentype_file}}

Like \luacallback{read_font_file}, but for opentype font files. The
\type{name} is a path name as returned by \luacallback{find_type1_file} 
or \type{kpse_find_file}.
...
\subsubsection{\luacallback{process_input_buffer}}
...
\subsubsection{\luacallback{token_filter}}
...
value to nil). See the description of \luacallback{token} for some
...
\subsubsection{\luacallback{buildpage_filter}}
...
\item \type{node} signals that the `head' node should be
replaced by this node
\item \type{boolean false} signals that the `head' node list should be
ignored and flushed from memory
\stopitemize

\subsubsection{\luacallback{pre_linebreak_filter}}

This callback is called just before \LUATEX\ starts converting a list
of nodes into a stack of \type{\hbox}es. The removal of any final skip
and the subsequent insertion of \type{\parfillskip} has not happened
yet at that moment.
...
\luacallback{pre_linebreak_filter}, some are for the
\luacallback{Xpack_filter} callbacks that will be explained in the next
two paragraphs.

\starttabulate[|l|p|]
\NC Value     \NC     Explanation \NC\FR
\NC              \NC       \NC\NR
...
\NC preamble     \NC  alignment preamble     \NC\NR
\NC align_set    \NC  alignment cell     \NC\NR
\NC fin_row      \NC  alignment row     \NC\NR
\stoptabulate

\subsubsection{\luacallback{hpack_filter}}
...
The \type{packtype} is either `additional' or `exactly'. If
`additional', then the \type{size} is a \type{\hbox spread ...}
argument. If `exactly', then the \type{size} is a \type{\hbox to ...}.
In both cases, the number is in scaled points.

\subsubsection{\luacallback{vpack_filter}}

This callback is called when \TEX\ is ready to start boxing some
vertical mode material. Math displays are ignored at the moment.

This function is very similar to the \luacallback{hpack_filter}. Besides
the fact that it is called at different moments, there is an extra
variable that matches \TEX's \type{\maxdepth} setting, but there is
no glyph count.
...
\subsubsection{\luacallback{pre_output_filter}}

This callback is called when \TEX\ is ready to start boxing the
box 255 for \type{\output}.
...
\subsubsection{\luacallback{start_run}}
...
\subsubsection{\luacallback{stop_run}}
...
\subsubsection{\luacallback{start_page_number}}
...
\subsubsection{\luacallback{stop_page_number}}
...
\subsubsection{\luacallback{show_error_hook}}
...
the values in the \lualib{status} table useful.
...
\subsubsection{\luacallback{define_font}}
...
\subsection{\luafunction{kpse.find_file}}
...
\subsection{\luafunction{kpse.set_program_name}}
...
\subsection{\luafunction{kpse.init_prog}}
...
\subsection{\luafunction{kpse.readable_file}}
...
due to aliases and system-specific handling under e.g. MS-DOS.

Returns nil if the file does not exist or is not readable.

\subsection{\luafunction{kpse.expand_path}}
...
\subsection{\luafunction{kpse.expand_var}}
...
\subsection{\luafunction{kpse.expand_braces}}
...
\subsection{\luafunction{kpse.var_value}}
...
\section{The \lualib{status} library}
...
\NC var_mem_max\NC               number of allocated words for nodes\NC \AR
\NC fix_mem_max\NC               number of allocated words for tokens\NC \AR
\NC fix_mem_end\NC               maximum number of used tokens\NC \AR
...
\NC output_active\NC             \type{true} if the \type{\output} routine is active\NC\AR
\stoptabulate
...
\NC table_version                \NC number   \NC indicates the \type{read_otf()} version\NC\NR
...
\NC possub                       \NC table    \NC (only if nonempty)\NC\NR
...
\NC script_lang_index      \NC number   \NC \NC\NR
\NC flags                  \NC number   \NC (only if nonzero)\NC\NR
\NC tag                    \NC string   \NC \NC\NR
\NC nccnt                  \NC number   \NC (only if nonzero)\NC\NR
\NC bccnt                  \NC number   \NC (only if nonzero)\NC\NR
\NC fccnt                  \NC number   \NC (only if nonzero)\NC\NR
...
\NC reversecoverage        \NC array     \NC Only if the parent's format is 'glyph'\NC\NR
\stoptabulate
...
\NC encodingbytes      \NC no       \NC no       \NC yes \NC number \NC default: depends on `format'\NC\NR
...
The rule is: if the font table has `encodingbytes' set to 2, then this
...
The current return value of \type{node.types()} is: \ctxlua {for _,v
...
A few node-typed userdata objects do not occur in the `normal' list of
...
\starttabulate[|l|l|l|]
\NC field              \NC type    \NC  Explanation \NC\FR
\NC width          \NC number  \NC \NC\NR
...
%\NC ref_count      \NC number  \NC reference count\NC\NR
\stoptabulate

These objects are reference counted, so there is actually an extra
field named `ref_count' as well, but that will become completely
...
functions in the \type{node} library, but for completeness, here is
...
first defined `attribute' item, whose `next' will point to the second
`attribute' item, etc.
...
an `attr' field. In the following tables \type{next} and \type{id} are
not explicitly mentioned.

\def\showfields#1%
  {\ctxlua{ for _,a in pairs(node.fields('#1')) do
            if not (a == 'id' or a == 'subtype' or a =='next' or a=='prev') then
            tex.print('\\type{'.. a .. '}')  end end }}
...
Valid fields: As for hlist, except that `shift' is a displacement
...
\NC width      \NC number  \NC  Rule size. The special value $-1073741824$
                                is used for `running' glue dimensions\NC\NR
\NC height     \NC number  \NC  ' '\NC\NR
\NC depth      \NC number  \NC  ' '\NC\NR
...
\NC top_skip   \NC node    \NC a pointer to the \type{\splittopskip} glue spec\NC\NR
\stoptabulate
...
\starttabulate[|l|l|l|]
\NC field          \NC type    \NC  Explanation \NC\FR
\NC subtype    \NC number  \NC  1 = `pre'\NC\NR
\NC attr       \NC node    \NC  \NC\NR
...
\starttabulate[|l|l|l|]
\NC field          \NC type    \NC  Explanation \NC\FR
\NC subtype    \NC number  \NC  0 = `on', 1 = `off'\NC\NR
\NC attr       \NC node    \NC  \NC\NR
...
\starttabulate[|l|l|p|]
\NC field          \NC type    \NC  Explanation \NC\FR
\NC subtype    \NC number  \NC  0 = \type{\skip},
                                1--18 = internal glue parameters,
                                100 = \type{\leaders},
                                101 = \type{\cleaders}, 
                                102 = \type{\xleaders}  \NC\NR
\NC attr       \NC node    \NC  \NC\NR
...
\NC subtype    \NC number  \NC  0 = from font, 
                                1 = from \type{\kern} or \type{\/}, 
                                2 = from \type{\accent}\NC\NR
\NC attr       \NC node    \NC  \NC\NR
\NC width      \NC number  \NC  \NC\NR
\stoptabulate
...
\type{node.whatsits()}: \ctxlua {for _,v in pairs(node.whatsits()) do
...
\NC left      \NC number  \NC value of \type{\lefthyphenmin}\NC\NR 
\NC right     \NC number  \NC value of \type{\righthyphenmin}\NC\NR 
\stoptabulate
...
\NC field         \NC type    \NC  Explanation \NC\FR
\NC attr      \NC node    \NC  \NC\NR
\NC mode      \NC number  \NC  The `mode' setting of this literal\NC\NR
\NC data      \NC string  \NC The \type{\pdfliteral} information\NC\NR 
\stoptabulate

\subsection{pdf_refobj nodes}
...
\subsection{pdf_refximage nodes}
...
\subsection{pdf_start_link nodes}
...
\subsection{pdf_start_thread nodes}
...
\subsection{pdf_end_thread nodes}
...
\subsection{pdf_snap_ref_point nodes}

Valid fields: \showfields{whatsit','pdf_snap_ref_point}
...
\subsection{pdf_snapy nodes}

Valid fields: \showfields{whatsit','pdf_snapy}


\starttabulate[|l|l|p|]
\NC field         \NC type    \NC  Explanation \NC\FR
\NC attr      \NC node    \NC  \NC\NR
\NC final_skip \NC number \NC \NC\NR
\NC spec      \NC node \NC pointer to a glue spec\NC\NR
\stoptabulate

\subsection{pdf_snapy_comp nodes}

Valid fields: \showfields{whatsit','pdf_snapy_comp}

\starttabulate[|l|l|p|]
\NC field         \NC type    \NC  Explanation \NC\FR
\NC attr      \NC node    \NC  \NC\NR
\NC comp_ratio \NC number \NC \NC\NR
\stoptabulate

\subsection{late_lua nodes}
...
\subsection{pdf_colorstack  nodes}
...
\subsection{pdf_setmatrix nodes}
...
\NC value    \NC number, string, node,  or table \NC \NC\NR
\stoptabulate
...
\item The \type{\endlinechar} can be either added (values 0 or more), or not
...
\item A few other experimental primitives are also provided without the
      extra  `pdf' prefix, so they can also be called like this:
\starttyping
    \primitive
    \ifprimitive
    \ifabsnum
...
\item The \type{\pdfprimitive} is bugfixed.

\item The \type{\pdftexversion} is set to 200.
...
\type{extra_mem_top} and \type{extra_mem_bot}). `Out of main memory' errors can still occur, 
...
directions. And also, the \type{\textdir} command is broken.

\item There is interference between rules and \type{\pdfliteral}. This
is also likely related to the Bidi algorithm.
...
% \item The five- and six character forms of \type{^^^^} are buggy.
...
\item Create callback for the automaticl creation of missing
characters in fonts
...
\item Switch to FontForge 2.0

\item Complete the attributes in math and switch the nodes to a double-linked list.

\item Move the `raw bytes' outside of the legal Unicode range, to
prevent collisiosn.
