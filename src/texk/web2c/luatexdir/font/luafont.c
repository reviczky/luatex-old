...
font_from_lua (lua_State *L, int f) {
  ...
  if (lua_isnumber(L,-1)) {
    set_font_type(f,lua_tonumber(L,-1));
  ...
  if(font_type(f) == virtual_font_type) {
    ...
    if (lua_istable(L,-1)) {
      n = lua_objlen(L,-1);
      if (n>255) {
	fprintf(stderr, "Too many local fonts");
	n = 255;
      }
      for (i=1;i<=n;i++) {
	lua_rawgeti(L,-1,i);
	...
	if (lua_istable(L,-1)) {
	  ...
	  /* TODO: fix this! the stack is messed up, otherwise this 
